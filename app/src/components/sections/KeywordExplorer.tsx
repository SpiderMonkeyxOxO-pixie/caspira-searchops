      ...
      {searched && rows.length > 0 && (
        <>
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: 'KEYWORDS FOUND', val: rows.length.toString(),             color: '#00d4ff' },
              { label: 'TOTAL VOLUME',   val: totalVol ? (totalVol/1000).toFixed(0)+'K' : '—', color: '#10b981' },
              { label: 'AVG KD',         val: avgKD ? avgKD+'%' : '—',            color: '#f59e0b' },
              { label: 'AVG CPC',        val: Number(avgCPC) ? '$'+avgCPC : '—',  color: '#7c3aed' },
            ].map(k => (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border text-[10px] text-muted font-mono-jarvis tracking-widest">
                    ...
                    <th className="text-right pb-2 font-medium pr-4">VOLUME</th>
                    <th className="text-left pb-2 font-medium w-32">KD</th>
                    <th className="text-center pb-2 font-medium">TREND</th>
                    <th className="text-right pb-2 font-medium">CPC</th>
                    <th className="text-center pb-2 font-medium">SRC</th>
                  </tr>
                </thead>
                <tbody>
                  {results.map((r, i) => (
