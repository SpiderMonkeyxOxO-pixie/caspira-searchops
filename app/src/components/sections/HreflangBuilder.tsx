...
function generateHTML(entries: HreflangEntry[]): string {
  const lines: string[] = ['<!-- Hreflang tags generated by Jarvis SEO -->']
  entries.forEach(e => {
...
function validateEntries(entries: HreflangEntry[]): { type: 'error' | 'warning' | 'pass'; msg: string }[] {
  ...
  if (entries.some(e => e.lang === 'en' && e.region === 'GB')) {
    issues.push({ type: 'pass', msg: 'en-GB (UK) entry present — important for UK casino SEO' })
  }

  const gbEntry = entries.find(e => e.lang === 'en' && e.region === 'GB')
  const ieEntry = entries.find(e => e.lang === 'en' && e.region === 'IE')
  if (gbEntry && !ieEntry) {
    issues.push({ type: 'warning', msg: 'You have en-GB but no en-IE — Ireland is a major iGaming market worth targeting' })
  }
  return issues
}
