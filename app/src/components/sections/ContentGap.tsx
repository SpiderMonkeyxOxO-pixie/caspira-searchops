...
Generate 6 realistic casino/gambling gaps (bonus pages, review content, game guides, geo pages). Opportunity: HIGH=kd<35, MED=moderate difficulty, LOW=hard to win.`,
    ...
    onSuccess: (data) => {
      if (data) {
        ...
        let parsedGaps: GapItem[] = []
        let parsedSummary = ''
        if (gapsMatch) { try { parsedGaps = JSON.parse(gapsMatch[1].trim()) as GapItem[] } catch { /* keep */ } }
        ...
        if (parsedGaps.length > 0) {
          save({
            ...
            label: `${own} vs ${c1}${c2 ? ` & ${c2}` : ''}`,
            sublabel: `${niche} · ${parsedGaps.filter(g=>g.opportunity==='HIGH').length} HIGH opp gaps`,
            own, c1, c2, niche,
            gaps: parsedGaps,
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === 'history' ? (
        <HistoryPanel
          records={records}
          onLoad={r => { setOwn(r.own); setC1(r.c1); setC2(r.c2); setNiche(r.niche); setGaps(r.gaps); setAiSummary(r.aiSummary); setTab('tool') }}
          onDelete={remove}
          onClear={clear}
          emptyText="No gap analysis history yet. Run the tool to save results."
        />
      ) : (
        <>
          {/* Inputs */}
