...
import { Sparkles, Info, ShoppingCart, FileText, Navigation, ChevronRight, AlertCircle, Download, History } from 'lucide-react'
...
export function IntentAnalyzer() {
  ...
  const { records, save, remove, clear } = useHistory<IntentRecord>('jarvis_intent_history')

  function saveResult(kwInput: string, resultRows: AnalysisRow[], isAi: boolean) {
    const kwList = kwInput.split('\n').filter(Boolean)
    save({
      id: crypto.randomUUID(),
      savedAt: new Date().toISOString(),
      label: `${kwList.length} keywords`,
      sublabel: `${isAi ? 'AI' : 'Rules'} · ${kwList.slice(0, 3).join(', ')}${kwList.length > 3 ? '…' : ''}`,
      input: kwInput,
      aiMode: isAi,
      rows: resultRows,
    })
  ...
  function analyzeLocal() {
    ...
    const result = buildRows(kwList)
    setRows(result)
    setAiMode(false)
    setAnalyzed(true)
    saveResult(input, result, false)
  }
...
- Nav = user looking for a specific brand/site`,
      ...
      setAiMode(true)
      setAnalyzed(true)
      saveResult(input, validated, true)
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === 'history' ? (
        <HistoryPanel
          records={records}
          onLoad={r => { setInput(r.input); setRows(r.rows); setAiMode(r.aiMode); setAnalyzed(true); setTab('tool') }}
          onDelete={remove}
          onClear={clear}
          emptyText="No intent analysis history yet. Run the analyzer to save results."
        />
      ) : (
        <>
          <Card>
